...
- (void)createPageControl{
    ...
    CGFloat pageControlY = self.bottomScrollView.origin.y - 20;
...
- (void)createTopScrollView{
    ...
    _faceArr = @[defaultModelArr,lxhModelArr,emojiModelArr,defaultModelArr,lxhModelArr,emojiModelArr];
...
- (void)createBottomScrollView{
     ...
     *  创建按钮
     */
    NSArray * nameArr = @[@"默认",@"浪小花",@"emoji",@"默认",@"浪小花",@"emoji"];
    _typeArr = nameArr;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(scrollView == self.topBigScrollView){
        NSUInteger facePageCount = (NSUInteger)(self.topBigScrollView.contentOffset.x / self.topBigScrollView.width);
        CGFloat currentScrX = _currentScrPage * scrollView.width;
        ...
        NSUInteger count = (NSUInteger)(scrollView.contentOffset.x / scrollView.width + 0.5);
